...
<div class="container">
    ...
    <form action="{{ route('tickets.update', ['ticket' => $ticket->id]) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="ticket_category_id" id="ticket_category_id" class="form-select" required>
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}" {{ old('ticket_category_id', $ticket->ticket_category_id) == $category->id ? 'selected' : '' }}>
                        {{ $category->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="ticket_priority_id" id="ticket_priority_id" class="form-select" required>
                @foreach ($priorities as $priority)
                    <option value="{{ $priority->id }}" {{ old('ticket_priority_id', $ticket->ticket_priority_id) == $priority->id ? 'selected' : '' }}>
                        {{ $priority->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <select name="ticket_status_id" id="ticket_status_id" class="form-select" required>
                @foreach ($statuses as $status)
                    <option value="{{ $status->id }}" {{ old('ticket_status_id', $ticket->ticket_status_id) == $status->id ? 'selected' : '' }}>
                        {{ $status->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" name="deadline" id="deadline" class="form-control" value="{{ old('deadline', $ticket->deadline->format('Y-m-d\TH:i')) }}" required>
            @error('deadline')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
